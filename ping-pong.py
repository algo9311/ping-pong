...
ball = GameSprite('pelota.jpg', 200, 200, 4, 50, 50) 
pared = Player('pares.jpg',25,200,4,30,100) 
segunda = Player('pares.jpg',550,200,4,30,100) 
...
while game: 
    ...
    if finish != True: 
        ...
        if sprite.collide_rect(pared, ball):
            if speed_x <0:
                speed_x *= -1
                ball.rect.left = pared.rect.right

        if sprite.collide_rect(segunda, ball):
            if speed_x >0:
                speed_x *= -1
                ball.rect.right = segunda.rect.left
